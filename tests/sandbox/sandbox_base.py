...
class SandboxedJusterTestCase(SandboxedNodeTestCase):
    ...
    def _deploy_pool(self, client):
        ...
        storage = generate_pool_storage(manager=pkh(self.manager))
    ...
    def _deploy_reward_program(self, client, juster_address):
        ...
        storage = dict(juster=juster_address, result=False)

        opg = contract.originate(initial_storage=storage)
        result = opg.send()
        self.bake_block()

        self.reward_program = self._find_contract_by_hash(
            client, result.hash()
        )
    ...
    def _create_simple_event(self, client, bets_time=10):
        ...
        opg = (
            client.contract(self.juster.address)
    ...
    def _provide_liquidity(
        ...
        opg = (
            user.contract(self.juster.address)
            .provideLiquidity(
                eventId=event_id,
    ...
    def _bet(
        ...
        opg = (
            user.contract(self.juster.address)
            .bet(
                eventId=event_id, bet=side, minimalWinAmount=minimal_win_amount
            )
            .with_amount(amount)
            .send()
        )
    ...
    def _withdraw(self, event_id=0, user=None, participant_address=None):
        ...
        opg = (
            user.contract(self.juster.address)
            .withdraw(eventId=event_id, participantAddress=participant_address)
            .send()
        )
