...
EXAMPLE_STORAGE = {
    'betsAboveEq': {('tz1MdaJfWzP5pPx3gwPxfdLZTHW6js9havos', 0): 75_000},
    'betsBelow': {},
    'depositedBets': {('tz1MdaJfWzP5pPx3gwPxfdLZTHW6js9havos', 0): 50_000},
    'events': {
        0: {
...
class EventModelTest(TestCase):
    ...
    def test_bet_pool_b_win(self):
        event = EventModel(
            fee=0, winning_pool='below', pool_a=4_000_000, pool_b=1_000_000
        )
    ...
    def test_bet_pool_b_lose(self):
        event = EventModel(
            fee=0, winning_pool='aboveEq', pool_a=2_000_000, pool_b=1_000_000
        )
        event.bet('user', 1_000_000, 'below', 1)
    ...
    def test_bet_with_fee(self):
        event = EventModel(
            fee=0.2, winning_pool='aboveEq', pool_a=1_000_000, pool_b=1_000_000
        )
    ...
    def test_from_storage_and_equal(self):
        created_event = EventModel.from_storage(
            storage=EXAMPLE_STORAGE, event_id=0, winning_pool='aboveEq'
        )
    ...
    def test_equal_not_equal(self):
        ...
        second_event.provide_liquidity(
            user='tz1RS9GoEXakf9iyBmSaheLMcakFRtzBXpWE', amount=100_000
        )
    ...
    def test_pool_views(self):
        ...
        for _ in range(100):
            ...
            self.assertAlmostEqual(
                event.pool_a_ratio() * event.pool_b_ratio(), 1
            )

            self.assertAlmostEqual(
                event.pool_a_expected() + event.pool_b_expected(), 1
            )
