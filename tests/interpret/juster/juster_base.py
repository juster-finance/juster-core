...
class JusterBaseTestCase(TestCase):
    ...
    def remove_none_values(self, storage):
        ...
        return {
            ...
            if is_should_be_cleaned(key, value)
            else value
    ...
    def check_storage_integrity(self, storage):
        ...
        for event_id, event in storage['events'].items():
            ...
            liquidity_above_eq_event = sum_ledger_by_event(
                pl_above_eq, event_id
            )
            ...
            pool_difference_check = (
                liquidity_above_eq_event
                - liquidity_below_event
                - wins_below_event
                + wins_above_eq_event
    ...
    def provide_liquidity(
        ...
        for outcome in ['aboveEq', 'below']:
            init_model = EventModel.from_storage(
                self.storage, self.id, outcome
            )
            init_model.provide_liquidity(
                participant, amount, expected_above_eq, expected_below
            )
            result_model = EventModel.from_storage(
                result.storage, self.id, outcome
            )
    ...
    def bet(self, participant, amount, bet, minimal_win):

        # Running transaction:
        transaction = self.contract.bet(
            eventId=self.id, bet=bet, minimalWinAmount=minimal_win
        ).with_amount(amount)
    ...
    def _check_withdrawals_sums(
        ...
        if closed_time is None:
            ...
            is_time_before_split = (
                self.current_time < closed_time + reward_fee_split_after
            )
        ...
        if (
            is_time_before_split
            or is_sender_equals_participant
            or is_force_majeure
        ):
    ...
    def new_event(self, event_params, amount):
        ...
        # Running transaction:
        result = (
            self.contract.newEvent(event_params)
            .with_amount(amount)
            .interpret(now=RUN_TIME, storage=self.storage, amount=amount)
        )
    ...
    def start_measurement(self, callback_values, source, sender):
        ...
        result = self.contract.startMeasurementCallback(
            callback_values
        ).interpret(
        ...
        self.assertEqual(
            event['measureOracleStartTime'], callback_values['lastUpdate']
        )
    ...
    def close(self, callback_values, source, sender):
        ...
        self.assertEqual(
            event['closedOracleTime'], callback_values['lastUpdate']
        )
    ...
    def setUp(self):
        ...
        self.contract = ContractInterface.from_file(
            join(dirname(__file__), CONTRACT_FN)
        )
