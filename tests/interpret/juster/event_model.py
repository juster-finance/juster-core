...
def calc_liquidity_bonus_multiplier(current_time, start_time, close_time):
...
def select_event(ledger, event_id):
    ...
    return {
        key[0]: value for key, value in ledger.items() if key[1] == event_id
    }
...
def calculate_diff(user, ledgers, split_pool):
    ...
    split_share = 0 if total_shares == 0 else split_pool * share / total_shares
...
def calculate_diffs(ledgers, split_pool):
    ...
    participants = set(
        participant for ledger in ledgers.values() for participant in ledger
    )
...
class EventModel:
    ...
    def provide_liquidity(self, user, amount, pool_a=0, pool_b=0):
        ...
        provided_a, provided_b, shares = self.calc_provided_shares(
            amount, pool_a, pool_b
        )
    ...
    def calc_bet_profit(self, amount, pool, time):
        ...
        bet_profit = calc_bet_return(
            top, bottom, amount, self.fee * multiplier
        )
        return bet_profit
    ...
    def distribute_between_providers(self, amount):
        ...
        for provider, shares in self.shares.items():
            ...
            self.diffs[provider] = self.diffs.get(provider, 0) + int(
                fraction * amount
            )
