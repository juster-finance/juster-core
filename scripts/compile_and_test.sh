# docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:0.23.0 compile-contract contracts/experimental/Token.ligo main > build/tz/token.tz
# docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:0.23.0 compile-contract contracts/experimental/Juster-B.ligo main > build/tz/juster_b.tz
