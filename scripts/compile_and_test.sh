...
docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:0.40.0 compile expression pascaligo lambda --init-file "contracts/lambdas/juster/reset_config.ligo" > build/lambdas/reset_new_event_config.tz
docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:0.40.0 compile expression pascaligo lambda --init-file "contracts/lambdas/juster/change_oracle.ligo" > build/lambdas/change_oracle.tz
docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:0.40.0 compile expression pascaligo lambda --init-file "contracts/lambdas/juster/trigger_pause_events.ligo" > build/lambdas/trigger_pause.tz
docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:0.40.0 compile expression pascaligo lambda --init-file "contracts/lambdas/juster/set_measurement_fees.ligo" > build/lambdas/set_measurement_fees.tz
