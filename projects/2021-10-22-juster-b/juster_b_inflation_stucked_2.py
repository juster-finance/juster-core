...
class JusterB:
    ...
    def withdraw_lock(self, lock_id):
        ...
        pools_for_deposit = lock.pools * deposit.shares / self.total_shares # * self.inflation
    ...
    def inflate(self, amount):
        ...
        rate = 1 if total_liquidity == 0 else 1 + amount / self.pools.max()
        # self.pools *= rate
        self.inflation /= rate
    ...
    def give_reward(self, agreement_id):
        ...
        if agreement.pool == self.get_win_pool():
            ...
            self.inflate(-agreement.delta)
            self.inflate(-agreement.amount)
            # self.inflate(-max(agreement.delta, agreement.amount))
        else:
            self.inflate(agreement.amount)
