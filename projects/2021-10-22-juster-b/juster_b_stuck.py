...
class JusterB:
    ...
    def insure(self, user, amount, pool):
        ...
        active_liquidity = 1 - self.locked_shares / self.total_shares
        active_pool_from = self.pools[ pool_from ] * active_liqudity
        active_pool_to = self.pools[ pool_to ] * active_liquiduty

        ratio = active_pool_from / (active_pool_to + amount)
        delta = ratio * amount
    ...
    def lock_liquidity(self, user, shares):
        ...
        # TODO: is it possible to reduce information inlock?
        lock = Lock(
            user=user,
            shares=shares,
            deposited=lock_deposit.deposited,
            pools={
                'for': self.pools['for'],
                'against': self.pools['against']
            },
    ...
    def withdraw_lock(self, lock_id):
        ...
        # for_pool_cut = lock.pools['for'] * lock.shares / lock.total_shares
        # against_pool_cut = lock.pools['against'] * lock.shares / lock.total_shares
        for_pool_cut = self.pools['for'] * lock.shares / self.total_shares
        against_pool_cut = self.pools['against'] * lock.shares / self.total_shares
