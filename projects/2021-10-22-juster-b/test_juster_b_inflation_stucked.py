...
def test_some_liquidity_removed_and_then_some_bet_placed():
    ...
    # assert jb.pools == Pools(12.5, 50)

    # A withdraws liquidity and no one interacts while it is locked:
    lock = jb.lock_liquidity('A', 40)
    jb.withdraw_lock(lock)
    assert jb.total_shares == 60
    # assert jb.pools == Pools(7.5, 30)

    # A returns 40% of deposit: 40 and accepts 40% of the losses (-20)
    jb.assert_balances_equal({'A': -100 + 40 - 20})

    # C makes another win with 7.5/60*30 = 3.75
    insurance_two = jb.insure('C', 30, 'against')
    # assert jb.pools == Pools(3.75, 60)

    # MAYBE: provider is not able to withdraw while there are some participants
    # in the contract that does not get reward?
    # And allow anyone to call this get reward?
    # And then track that earliest agreements.date is more than lock.date?
    # - but then how to update this date?

    # This time A withdraws first [LOOKS LIKE THIS IS WRONG, A CAN'T WITHDRAW FIRST]:
    '''
    ...
    # Finnaly withdraws A:
    # accepts another 60% of losses (-30) + deposit 60 and -3.75 for C = 26.25
    lock = jb.lock_liquidity('A', 60)
    jb.withdraw_lock(lock)
